<template>
    <div>
        ...
        <!-- <pagination :data="Object.fromEntries(expenses)" @pagination-change-page="allExpense">
                                    ...
                                    <table id="tablehome" class="table align-items-center table-flush">
                                        ...
                                        <tr v-for="expense in filtersearch" :key="expense.id">
                                            ...
                                            <td>
                                                ...
                                                <a @click="cancelExpense(expense.id)" class="btn btn-sm btn-danger"><font color="#fffffff">Cancel</font></a>
                                            </td>
                                        </tr>
                        ...
                        <div class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                            ...
                            <div class="row"> 
                                ...
                                <table class="table align-items-center table-flush">
                                        ...
                                        <tbody>
                                        <tr v-for="getexpense in getfiltersearch" :key="getexpense.id">
                                            <td>{{ getexpense.details }}</td>
                                            <td>{{ getexpense.amount }}</td>
                                            <td>{{ getexpense.expense_date }}</td>
                                            <td>
                                                <router-link :to="{name: 'edit-expense', params:{id:getexpense.id}}" class="btn btn-sm btn-primary">Edit</router-link>

                                                <a @click="cancelExpense(getexpense.id)" class="btn btn-sm btn-danger"><font color="#fffffff">Cancel</font></a>
                                            </td>
                                        </tr>
                                        <tr class="bg-dark text-white">
                                            <td>Totals:</td>
...
computed:{
    ...
    totalAmt(){
      let total = 0;  
      for(let p of this.expenses) 
      {    total += parseFloat(p.amount);  }  
      return  parseFloat(total).toFixed(2);
    },
    totalAmtOthers(){
      let total = 0;  
      for(let p of this.getexpenses) 
      {    total += parseFloat(p.amount);  }  
      return  parseFloat(total).toFixed(2);
    },
},
...
methods: {
    ...
    axios.post('/api/myexpense',param1)
    .then(({data}) => {this.expenses = data;console.log(this.expenses);})
    .catch()
    },
    deleteExpense(id){
        Swal.fire({
    ...
    cancelExpense(id){
        ...
        if (result.isConfirmed) {
            axios.post('/api/cancelexpense/'+id)
            .then(()=>{
                ...
                this.getexpenses  = this.getexpenses.filter(getexpense =>{
                    return getexpense.id != id
                })
